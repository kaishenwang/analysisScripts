...
XT = np.concatenate((array[:10000,1:len(array[0])],array[15000:,1:len(array[0])]))
YT = np.concatenate((array[:10000,0],array[15000:,0]))
XV = array[10000:15000,1:len(array[0])]
YV = array[10000:15000,0]
...
clf.fit(X,Y)
#pickle.dump(clf, open('randomeForest.pickle', 'wb'))
#clf.fit(XT, YT)
'''
...
for i in range(len(YG)):
    if YV[i] != YG[i]:
        print ("At %d get %d instead of %d"%(i+10002, YG[i], YV[i]))
'''
with open('randomeForest.pickle', 'rb') as f:
    clf2 = pickle.load(f)
YG = clf2.predict(XV)
for i in range(len(YG)):
    if YV[i] != YG[i]:
